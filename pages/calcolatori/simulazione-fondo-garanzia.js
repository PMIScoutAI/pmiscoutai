...
const FondoGaranziaCalculator = () => {
    ...
    const validateStep = () => {
        const errors = {};
        if (currentStep === 1 && !formData['data-costituzione']) errors['data-costituzione'] = true;
        if (currentStep === 2) {
    ...
    const estimateCreditClass = (isStartup, data) => { const { settore, fatturato, ebitda, pfn, patrimonioNetto, oneriFinanziari } = data; if (isStartup) { return { class: 3, notes: "Per le start-up, la valutazione si basa sul business plan. Si assume una classe di merito prudenziale." }; } if (fatturato === 0 || patrimonioNetto === 0) { return { class: 4, notes: "Dati economici insufficienti per una stima precisa. Si assume una classe di merito prudenziale." }; } let score = 0; const notes = []; const pfn_su_pn = patrimonioNetto !== 0 ? pfn / patrimonioNetto : 10; const pfn_su_ebitda = ebitda > 0 ? pfn / ebitda : 10; const oneri_su_ebitda = ebitda > 0 ? oneriFinanziari / ebitda : 10; const ebitda_margin = fatturato !== 0 ? ebitda / fatturato : 0; switch(settore) { case 'industria': case 'servizi': if (pfn_su_pn < 2) score += 2; else if (pfn_su_pn > 5) score -= 2; else score -=1; notes.push(`Leva (PFN/PN) di ${pfn_su_pn.toFixed(2)}`); if (pfn_su_ebitda < 3) score += 2; else if (pfn_su_ebitda > 6) score -= 2; else score -=1; notes.push(`Sostenibilità Debito (PFN/EBITDA) di ${pfn_su_ebitda.toFixed(2)}`); if (ebitda_margin > 0.1) score += 2; else if (ebitda_margin < 0.03) score -= 2; else score +=1; notes.push(`Redditività (EBITDA Margin) del ${(ebitda_margin * 100).toFixed(1)}%`); break; case 'commercio': if (pfn_su_pn < 3) score += 2; else if (pfn_su_pn > 6) score -= 2; else score -=1; notes.push(`Leva (PFN/PN) di ${pfn_su_pn.toFixed(2)}`); if (pfn_su_ebitda < 4) score += 2; else if (pfn_su_ebitda > 7) score -= 2; else score -=1; notes.push(`Sostenibilità Debito (PFN/EBITDA) di ${pfn_su_ebitda.toFixed(2)}`); if (ebitda_margin > 0.05) score += 2; else if (ebitda_margin < 0.01) score -= 2; else score +=1; notes.push(`Redditività (EBITDA Margin) del ${(ebitda_margin * 100).toFixed(1)}%`); break; case 'edilizia': case 'immobiliare': if (pfn_su_pn < 1.5) score += 2; else if (pfn_su_pn > 4) score -= 2; else score -=1; notes.push(`Leva (PFN/PN) di ${pfn_su_pn.toFixed(2)}`); const pn_su_fatturato = fatturato !== 0 ? patrimonioNetto / fatturato : 0; if (pn_su_fatturato > 0.3) score += 2; else if (pn_su_fatturato < 0.1) score -= 2; else score +=1; notes.push(`Solidità (PN/Fatturato) di ${pn_su_fatturato.toFixed(2)}`); if (ebitda_margin > 0.12) score += 2; else if (ebitda_margin < 0.05) score -= 2; else score +=1; notes.push(`Redditività (EBITDA Margin) del ${(ebitda_margin * 100).toFixed(1)}%`); break; } if (patrimonioNetto < 0) { score -= 3; notes.push("Patrimonio Netto negativo (forte penalità)."); } if (ebitda < 0) { score -= 3; notes.push("EBITDA negativo (forte penalità)."); } let finalClass; if (score >= 4) finalClass = 1; else if (score >= 2) finalClass = 2; else if (score >= 0) finalClass = 3; else if (score >= -3) finalClass = 4; else finalClass = 5; return { class: finalClass, notes: `Stima basata su: ${notes.join(', ')}.` }; };
    const getCoveragePercentage = (classeDiMerito, finalita, durataMesi) => { if (classeDiMerito > 4) return 0; if (finalita === 'sabatini') return coverageTable.sabatini[classeDiMerito - 1]; if (finalita === 'microcredito') return coverageTable.microcredito[classeDiMerito - 1]; if (finalita === 'risanamento') return coverageTable.risanamento[classeDiMerito - 1]; if (finalita === 'investimento') return coverageTable.investimenti[classeDiMerito - 1]; let operationType; if (durataMesi <= 12) operationType = 'fin_fino_12_mesi'; else if (durataMesi <= 36) operationType = 'fin_oltre_12_fino_36_mesi_senza_pa'; else operationType = 'fin_oltre_36_mesi_senza_pa'; return coverageTable[operationType][classeDiMerito - 1]; };
    ...
    const handleSubmit = (e) => {
        ...
        let { class: classeDiMerito, notes: classNotes } = estimateCreditClass(isStartup, numericData); let finalNotes = [classNotes];
        if (formData['pregiudizievole-fallimento']) { classeDiMerito = 5; finalNotes.push("La presenza di procedure concorsuali determina la non ammissibilità."); } else if (formData['pregiudizievole-grave']) { const classeOriginale = classeDiMerito; classeDiMerito = Math.min(5, classeDiMerito + 2); finalNotes.push(`La presenza di eventi pregiudizievoli gravi ha causato un declassamento da classe ${classeOriginale} a ${classeDiMerito}.`); }
        ...
        <div className="bg-white p-6 md:p-8 rounded-2xl shadow-lg">
            ...
            {!results ? (
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className={`form-section ${currentStep === 1 ? 'active' : ''}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label htmlFor="forma-giuridica" className="block text-sm font-medium text-gray-700 mb-1">Forma Giuridica</label>
                                    <select id="forma-giuridica" value={formData['forma-giuridica']} onChange={handleInputChange} className="w-full p-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500">
                                ...
                                <div>
                                    ...
                                    <input type="date" id="data-costituzione" value={formData['data-costituzione']} onChange={handleInputChange} required className={`w-full p-3 border rounded-lg shadow-sm ${formErrors['data-costituzione'] ? 'border-red-500' : 'border-gray-300'}`} />
                                </div>
                                <div className="md:col-span-2">
                                    <label htmlFor="settore-ateco" className="block text-sm font-medium text-gray-700 mb-1">Settore ATECO</label>
                                    <select id="settore-ateco" value={formData['settore-ateco']} onChange={handleInputChange} className="w-full p-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500">
                                        <option value="industria">Industria</option> <option value="commercio">Commercio</option> <option value="servizi">Servizi</option> <option value="edilizia">Edilizia</option> <option value="immobiliare">Immobiliare</option>
                                    </select>
                                </div>
                                <div className="md:col-span-2">
                                    <fieldset className="mt-2">
                                        <legend className="text-sm font-medium text-gray-700">Caratteristiche Impresa</legend>
                                        <div className="mt-2 space-y-2">
                                            <div className="flex items-center"><input id="impresa-femminile" type="checkbox" checked={formData['impresa-femminile']} onChange={handleInputChange} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" /><label htmlFor="impresa-femminile" className="ml-3 block text-sm text-gray-800">Impresa Femminile</label></div>
                                            <div className="flex items-center"><input id="impresa-giovanile" type="checkbox" checked={formData['impresa-giovanile']} onChange={handleInputChange} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" /><label htmlFor="impresa-giovanile" className="ml-3 block text-sm text-gray-800">Impresa Giovanile</label></div>
                                        </div>
                                    </fieldset>
                                </div>
                            </div>
                            <div className="mt-8 text-right"><button type="button" onClick={() => nextStep(2)} className="bg-blue-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-blue-700">Prosegui &rarr;</button></div>
                        </div>
                        ...
                        <div className={`form-section ${currentStep === 2 ? 'active' : ''}`}>
                            ...
                            <div className="mt-8 flex justify-between"><button type="button" onClick={() => prevStep(1)} className="bg-gray-200 text-gray-800 font-bold py-3 px-6 rounded-lg hover:bg-gray-300">&larr; Indietro</button><button type="button" onClick={() => nextStep(3)} className="bg-blue-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-blue-700">Prosegui &rarr;</button></div>
                        </div>
                            ...
                            <h3 className="text-xl font-semibold mb-4 text-center">Eventi Pregiudizievoli</h3>
                             <fieldset>
                                <div className="space-y-2">
                                    <div className="flex items-center"><input id="pregiudizievole-fallimento" type="checkbox" checked={formData['pregiudizievole-fallimento']} onChange={handleInputChange} className="h-4 w-4 text-red-600 border-gray-300 rounded focus:ring-red-500" /><label htmlFor="pregiudizievole-fallimento" className="ml-3 block text-sm text-gray-800">Presenza di procedure concorsuali</label></div>
                                    <div className="flex items-center"><input id="pregiudizievole-grave" type="checkbox" checked={formData['pregiudizievole-grave']} onChange={handleInputChange} className="h-4 w-4 text-orange-600 border-gray-300 rounded focus:ring-orange-500" /><label htmlFor="pregiudizievole-grave" className="ml-3 block text-sm text-gray-800">Presenza di Ipoteca Giudiziale o Pignoramento</label></div>
                                </div>
                            </fieldset>
                            <div className="mt-8 flex justify-between"><button type="button" onClick={() => prevStep(2)} className="bg-gray-200 text-gray-800 font-bold py-3 px-6 rounded-lg hover:bg-gray-300">&larr; Indietro</button><button type="submit" className="bg-green-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-green-700">Calcola Risultato</button></div>
                        </div>
                    </form>
                </>
            ) : (
                <div>
                    ...
                    <div className={`result-card ${results.percentuale > 0 ? 'result-card-green' : 'result-card-red'} bg-white p-6 rounded-lg shadow-md mb-6 text-center`}>
                        <p className={`text-2xl font-bold ${results.percentuale > 0 ? 'text-green-600' : 'text-red-600'} flex items-center justify-center`}>{results.percentuale > 0 ? 'Ammissibilità Indicativa: POSITIVA' : 'Ammissibilità Indicativa: NEGATIVA'}</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                        ...
                        <div className="bg-white p-4 rounded-lg shadow"><p className="text-sm text-gray-500">Importo Garantito
