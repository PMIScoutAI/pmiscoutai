...
// Nota: Il Layout non è più importato qui ma gestito dalla pagina che renderizza questo componente.


// --- DATI, CONFIGURAZIONE E VALORI DI DEFAULT ---
...
const emptyResults = { fairMarketValue: 0, conservativeValue: 0, optimisticValue: 0, evRevenue: 'N/A', evEbitda: 'N/A', peRatio: 'N/A', revenueGrowth: 0, ebitdaGrowth: 0, qualityScore: 0, riskScore: 0, liquidityDiscount: 0 };
...
const performCalculation = (formData) => {
    ...
    if (data.revenue === 0 || !industryData) return emptyResults;
    ...
    // FIX: Rimosso il malus per dimensione, mantenendo solo lo sconto di liquidità per evitare "double counting".
    ...
    // FIX: Aggiunto clamp per il fattore di aggiustamento
    ...
    return { ...emptyResults, ...finalResults };
};

// --- COMPONENTI UI RIUTILIZZABILI (REVISIONATI) ---
...
const FormField = ({ config, value, onChange }) => { const { id, label, type, options, placeholder } = config; const commonProps = { id, value, onChange, className: type === 'select' ? 'form-select' : 'form-input' }; return ( <div className="form-group"> <label className="form-label" htmlFor={id}>{label}</label> {type === 'select' ? ( <select {...commonProps}>{options.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select> ) : ( <input type="number" min="0" step="1" placeholder={placeholder} {...commonProps} /> )} </div> ); };
...
const ValutazioneAziendaleCalculator = () => {
    ...
    // FIX: Caricamento dinamico della libreria XLSX solo client-side
    useEffect(() => {
        import('xlsx').then(mod => {
            setXLSXLib(mod);
        }).catch(err => console.error("Failed to load xlsx library", err));
    }, []);

    const results = useMemo(() => performCalculation(formData), [formData]);
    
    // FIX: Sanitizzazione dell'input numerico
    const handleInputChange = (e) => {
      const { id, value, type } = e.target;
      let sanitizedValue = value;
      if (type === 'number') {
        sanitizedValue = value.replace(/,/g, '.'); // Sostituisce la virgola con il punto
    ...
    // FIX: Error handling nel fetch
    const loadUserAnalyses = async () => {
      if (!user?.email) return;
      setLoadingCompanies(true);
      try {
        const res = await fetch(`/api/user-analyses?email=${encodeURIComponent(user.email)}`);
    ...
    const loadCompanyDataFromAnalysis = async (analysis) => {
      try {
        const res = await fetch(`/api/company-xbrl-details?session_id=${analysis.session_id}`);
        if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
        const data = await res.json();
        if (data.success) {
          setFormData(prev => ({ ...prev, ...data.mappedData }));
    ...
    const handleExportExcel = () => {
        if (!XLSXLib) {
            console.error("Libreria XLSX non ancora caricata.");
            return;
        }
        const { utils, writeFile } = XLSXLib;
    ...
    // FIX: Formattatore data localizzato
    const dateFormatter = new Intl.DateTimeFormat('it-IT', { dateStyle: 'medium', timeZone: 'Europe/Rome' });
                        ...
                        <div style={{maxHeight: '300px', overflowY: 'auto'}}>
                          {availableCompanies.length > 0 ? availableCompanies.map((analysis) => (
                            <div key={analysis.session_id} className="company-list-item" onClick={() => loadCompanyDataFromAnalysis(analysis)}>
                              <div><strong>{analysis.company_name}</strong><br /><small>{dateFormatter.format(new Date(analysis.created_at))}</small></div>
...
const AppLayout = ({ children }) => {
    ...
    return (
        <>
            <style>{`
                /* Stili Globali e di Layout - Da spostare in globals.css */
                ...
                .sidebar-header h1 { font-size: 1.5rem; font-weight: bold; color: var(--primary-color); }
                .sidebar-nav { padding: 1rem; }
                .nav-link { display: flex; align-items: center; padding: 0.75rem 1rem; border-radius: 0.5rem; margin-bottom: 0.25rem; color: #475569; font-weight: 500; text-decoration: none; }
                ...
                @media (max-width: 1024px) { .content-grid { grid-template-columns: 1fr; } .results-card { position: static; } }
                ...
                .form-input, .form-select { width: 100%; padding: 0.75rem 1rem; border: 1px solid #cbd5e1; border-radius: 0.5rem; transition: all 0.2s ease; }
                .form-input:focus, .form-select:focus { outline: none; border-color: var(--primary-color); box-shadow: 0 0 0 2px #c7d2fe; }
                ...
                .scenarios, .metrics-section { margin-bottom: 1.5rem; }
                ...
                /* Stili di Stampa */
                @media print {
                  .app-layout > .sidebar, .app-layout .header-mobile, .results-card .button-group { display: none; }
                  body * { visibility: hidden; }
                  .printable-area, .printable-area * { visibility: visible; }
                  .printable-area { position: absolute; left: 0; top: 0; width: 100%; padding: 20px; margin: 0; }
            ...
            <div className="app-layout">
                <aside className={`sidebar ${isSidebarOpen ? 'open' : ''}`}>
                    ...
                    <nav className="sidebar-nav">
                        {navLinks.map((link) => (
                            <Link key={link.text} href={link.href} className={`nav-link ${link.active ? 'active' : ''}`}>
                ...
                <div className="main-content">
                    <header className="header-mobile">
                        <button onClick={() => setIsSidebarOpen(true)} aria-label="Apri menu"><Icon path={icons.menu} /></button>
                        <h1>PMIScout</h1>
                        <div/>
                    </header>
