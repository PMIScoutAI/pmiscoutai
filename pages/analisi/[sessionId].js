...
// Versione con import corretti, script Outseta e UI completa
    ...
    const fetchAndSubscribe = async () => {
        ...
        try {
            ...
            if (sessionResult.status === 'completed') {
                ...
                // Setup realtime listener solo se l'analisi è in corso
                if (channelRef.current) supabase.removeChannel(channelRef.current);
                const channel = supabase
                    .channel(`session_${sessionId}`)
                    .on('postgres_changes', { event: '*', schema: 'public', table: 'checkup_sessions', filter: `id=eq.${sessionId}` }, (payload) => fetchAndSubscribe())
                    .subscribe((status, err) => {
                        if (status === 'SUBSCRIBED') console.log(`Connesso al canale: session_${sessionId}`);
                        if (err) console.error('Errore sottoscrizione realtime:', err);
                    });
  ...
  const renderContent = () => {
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Qui puoi aggiungere le altre sezioni del report (SWOT, metriche, etc.) */}
        </div>
    );
  };
