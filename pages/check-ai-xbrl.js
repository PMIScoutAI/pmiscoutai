...
// VERSIONE 13.0 (Fix Estrazione Nome Azienda)
// - REFACTORING: Aggiunta una funzione `findCompanyName` dedicata e più robusta per l'estrazione del nome dell'azienda, come da richiesta.
// - La logica principale ora usa la nuova funzione con un fallback a quella precedente per massima affidabilità.
...
const getSectorInfo = async (divisionCode, sessionId) => {
    ...
    try {
        const { data, error } = await supabase.from('ateco_macro_map').select('macro_sector, macro_sector_2, notes').eq('ateco_code', divisionCode).single();
        if (error || !data) {
...
// 🔧 FIX 1: NOME AZIENDA - Funzione specifica
            ...
            for (let j = 0; j < Math.min(row.length, 6); j++) {
                ...
                if (cellValue === term.toLowerCase()) {
                    for (let k = j + 1; k < row.length; k++) {
                        ...
                        if (nameValue && nameValue.length > 2 && !nameValue.match(/^[T0-9.]+$/) && !nameValue.match(/^\d+$/) && nameValue !== 'si' && nameValue !== 'no') {
...
export default async function handler(req, res) {
  ...
  console.log(`[${sessionId}] Avvio analisi XBRL (versione 13.0).`);
  ...
  try {
    ...
    // IMPLEMENTAZIONE della nuova logica di ricerca nome azienda
